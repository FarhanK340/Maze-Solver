def maze_solver(maze):
    ...
    def dfs(current_position, current_path, intervals, maze , visited, memo):
        ...
        state = (current_position, current_path)
        ...
        for direction in ['N', 'W', 'S', 'E']:
            ...
            if  next_position not in visited and valid(next_position,current_position, direction, maze):
                result = dfs(next_position, current_path + direction, intervals, maze, visited.copy(), memo)
                if result:
                    memo[state] = result
                    return result
        
        visited.remove(current_position)
        memo[state] = current_path
        return None
    ...
    if intervals == 0:
        path = dfs(start, "", intervals, maze, set(), memo)
    ...
    while intervals < 20:
        ...
        while queue:
            ...
            if current_position == end:
                return current_path
            path = dfs(current_position, current_path, intervals, maze, set(current_visited), memo)
